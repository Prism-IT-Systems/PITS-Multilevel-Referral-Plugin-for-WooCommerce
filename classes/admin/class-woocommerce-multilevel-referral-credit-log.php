...
if ( ! class_exists( 'WooCommerce_Multilevel_Referral_Credit_Log' ) ) :
	/**
	 * WooCommerce Multilevel Referral Credit Log Table Class.
	 *
	 * @package Multilevel_Referral_Plugin_For_WooCommerce
	 * @since   2.28.1
	 */
	class WooCommerce_Multilevel_Referral_Credit_Log extends WP_List_Table {
		/**
		 * Constructor.
		 */
		...
		public function column_default( $item, $column_name ) {
			switch ( $column_name ) {
				...
				case 'user_id':
					$user_name = ucwords( get_user_meta( $item[ $column_name ], 'first_name', true ) . ' ' . get_user_meta( $item[ $column_name ], 'last_name', true ) );
					return $user_name ? $user_name : get_user_meta( $item[ $column_name ], 'nickname', true );
				default:
					return $item;
			}
		}
		...
		public function get_columns() {
			$columns = array(
				...
				'user_id'  => __( 'Name', 'multilevel-referral-plugin-for-woocommerce' ),
				'credits'  => __( 'Earned Credits', 'multilevel-referral-plugin-for-woocommerce' ),
				'redeems'  => __( 'Redeemed Credits', 'multilevel-referral-plugin-for-woocommerce' ),
			);
			return $columns;
		}
		/**
		...
		public function prepare_items() {
			...
			$total_items   = $search_result ? 0 : $woocommerce_multilevel_referral_program->record_count( null, true, $user_id, $order_id );
			...
			$this->items = $search_result ? 0 : $woocommerce_multilevel_referral_program->select_all( $per_page, $current_page, $user_id, $order_id );
		}
		/**
		 * Search box.
		 *
		...
		public function search_box( $text, $input_id ) {
			?>
		<p class="search-box">
			<?php wp_nonce_field( 'woocommerce_multilevel_referral_search_log_action', 'woocommerce_multilevel_referral_search_log_nonce' ); ?>
